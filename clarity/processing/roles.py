...
POLICY: Dict[str, Tuple[Optional[str], int]] = {
    "equipment": ("4x", 1024),
    "accessory": ("4x", 1024),
    "weapon": ("4x", 1024),
    "monster": ("2x", 2048),
...
def top_tier(
    family: str, w: int, h: int, override: Optional[str] = None
) -> Optional[str]:
...
def process(
    ...
    """
    out = (
        process_top(engine, role, family, rgba, src_fmt, top).astype(np.float32) / 255.0
    )
    result = {top: _u8(out)}
    cur = out
